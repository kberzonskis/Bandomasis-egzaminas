...
import { ContainersContext } from '../../context/containers/ContainersContext.js';
...
export function AdminBoxForm({ api, method, box }) {
    ...
    const [hours, setHours] = useState(box?.duration_in_minutes ? (box.duration_in_minutes - box.duration_in_minutes % 60) / 60 : 0);
    const [minutes, setMinutes] = useState(box?.duration_in_minutes ? box.duration_in_minutes % 60 : 0);
    const [containerId, setcontainerId] = useState(box?.container_id ?? 0);
    const [releaseDate, setReleaseDate] = useState(box?.release_date ? box.release_date.slice(0, box.release_date.indexOf('T')) : '');
    const [rating, setRating] = useState(box?.rating ?? 50);
        ...
        <>
            <form onSubmit={handleImageFormSubmit} className="col-12 col-md-9 col-lg-6 mt-5">
                <img id="img_preview" className="d-block w-100 object-fit-contain"
                    ...
                    src={img ? (SERVER_ADDRESS + '/img/boxes/' + img) : defaultImg} alt="box thumbnail" />
                <p id="img_path">{img}</p>
            ...
            <form onSubmit={handleMainFormSubmit} className="col-12 col-md-9 col-lg-6 mt-5">
                ...
                <div className="mb-3">
                    ...
                    <select onChange={e => setcontainerId(e.target.value)} value={containerId} className="form-select" id="container">
                        ...
                        {adminContainers.map(cat => <option key={cat.id} value={cat.id}>{cat.title}</option>)}
                    </select>
                </div>
                <div className="mb-3">
